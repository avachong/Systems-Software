...
{
  struct Block *ptr_block = sbrk(size + sizeof(struct Block));
  return ptr_block+1;
}
...
void *my_malloc(int size)
{
  ...
  void *temp = sbrk(size + sizeof(struct Block));

  if (head != NULL){
	while(current->next != NULL){
	  current = current->next;
	}
	struct Block *newTemp = temp;
	newTemp->size = size + sizeof(struct Block);
	newTemp->occ = 1;
        newTemp->prev = current;
  ...
  return temp + sizeof(struct Block);
}
...
{
  struct Block *temp = data - sizeof(struct Block);
  temp->occ =0;
}
